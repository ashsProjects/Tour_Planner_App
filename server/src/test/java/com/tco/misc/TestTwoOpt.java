...
public class TestTwoOpt {
    ...
    @BeforeEach
    public void createConfigForTwoOpt() {
    ...
    public void checkCorrectValueForTen() throws BadRequestException {
        Places optimizedPlaces = optimizer.construct(places, earthRadius, formula, response);
        int[] correct = {0, 1, 3, 9, 8, 6, 2, 5, 7, 4};
    ...
    @Test 
    @DisplayName("ayushad: Already optimized")
    public void checkPreOptimizedRoute() throws BadRequestException {
        Places testPlaces = new Places();
        testPlaces.add(new Place("44.09", "-108.46"));
        testPlaces.add(new Place("45.23","-90.18"));
        testPlaces.add(new Place("33.28", "-105.82"));
        testPlaces.add(new Place("35.32", "-86.84"));
        Places optimizedPlaces = optimizer.construct(testPlaces, earthRadius, formula, response);
        int[] correct = {0, 2, 3, 1};
        //assertEquals(optimizedPlaces, testPlaces);
        for (int i = 0; i < optimizedPlaces.size(); i++) {
            assertEquals(optimizedPlaces.get(i), testPlaces.get(correct[i]));
        }
    }
}
